...
class FeatureExtraction:
    ...
    def WebsiteTraffic(self):
        try:                                                                                               #
            rank = BeautifulSoup(urllib.request.urlopen("http://data.alexa.com/data?cli=10&dat=s&url=" + self.url, timeout=REQUEST_TIMEOUT).read(), "xml").find("REACH")['RANK']
            if (int(rank) < 100000):
                return 1
            return 0
        except :
    ...
    def PageRank(self):
        try:
            prank_checker_response = requests.post("https://www.checkpagerank.net/index.php", {"name": self.domain}, timeout=REQUEST_TIMEOUT)
    ...
    def GoogleIndex(self):
        try:
            results = list(search(self.url, num_results=3, timeout=REQUEST_TIMEOUT))
