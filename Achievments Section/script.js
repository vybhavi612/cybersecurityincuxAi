...
(function () {
  ...
  /* ── Animate progress bars + percentage counters ─────────── */
  function animateTrackerCards() {
    document.querySelectorAll('.tracker-card').forEach(card => {
      const fill   = card.querySelector('.tracker-progress__fill');
      const pctEl  = card.querySelector('.tracker-progress__pct');
      const target = parseInt(card.dataset.progress, 10) || 0;

      // Animate bar width
      if (fill) {
        setTimeout(() => { fill.style.width = (fill.dataset.width || target) + '%'; }, 250);
      }
  ...
  if (trackerSection && 'IntersectionObserver' in window) {
    const obs = new IntersectionObserver(entries => {
      entries.forEach(entry => { if (entry.isIntersecting) { initTracker(); obs.unobserve(entry.target); } });
    }, { threshold: 0.15 });
    obs.observe(trackerSection);
  } else {
  ...
  /* ════════════════════════════════════════════════════════
     TASK MANAGEMENT MODULE  (localStorage persisted)
  ════════════════════════════════════════════════════════ */

  const TASKS_STORAGE_KEY = 'studentHub_tasks_v1';

  /* ── Default seed tasks (used only on first run) ─────────── */
  const defaultTasks = [
    { id: 'task-1', text: 'Finish Classification Study', completed: false, createdAt: Date.now() - 2 * 86400000 },
    { id: 'task-2', text: 'Attend Mentor Meeting',        completed: false, createdAt: Date.now() - 1 * 86400000 },
    { id: 'task-3', text: 'Submit Weekly Report',         completed: false, createdAt: Date.now() },
  ];
  ...
  function saveTasks(tasks) {
    try {
      ...
      showToast('⚠️ Could not save tasks (storage unavailable)', 3000);
    }
  }
  ...
  function formatTaskDate(timestamp) {
    ...
    return 'Added ' + date.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' });
  }
  ...
  function renderTaskList() {
    ...
    const emptySub = document.getElementById('taskEmptySub');
    if (!listEl) return;
    ...
    if (filtered.length === 0) {
      ...
      if (emptyEl) emptyEl.hidden = false;
      if (emptySub) {
        if (tasks.length === 0) emptySub.textContent = 'Add a new task to get started.';
        else if (taskFilter === 'active') emptySub.textContent = 'All tasks are completed. Great job!';
        else if (taskFilter === 'completed') emptySub.textContent = 'No completed tasks yet.';
      }
    } else {
      if (emptyEl) emptyEl.hidden = true;
      listEl.innerHTML = filtered.map(task => `
        <li class="task-card ${task.completed ? 'completed' : ''}" data-id="${task.id}">
          <button class="task-card__checkbox" type="button" aria-label="${task.completed ? 'Mark as active' : 'Mark as complete'}: ${escapeHTML(task.text)}" aria-pressed="${task.completed}">
          ...
          <div class="task-card__body">
            <span class="task-card__text">${escapeHTML(task.text)}</span>
            <span class="task-card__date">${formatTaskDate(task.createdAt)}</span>
          </div>
          <button class="task-card__delete" type="button" aria-label="Delete task: ${escapeHTML(task.text)}">
  ...
  function renderTaskCounts() {
    ...
    set('countCompleted', completed);

    const clearBtn = document.getElementById('clearCompletedBtn');
    if (clearBtn) clearBtn.disabled = completed === 0;
  }
  ...
  function renderTaskProgress() {
    ...
    const ringEl = document.getElementById('taskRingFill');
    ...
    if (ringEl) {
      ...
      ringEl.style.strokeDashoffset = CIRC - (pct / 100) * CIRC;
      // Colour shift based on progress
      if (pct === 100 && total > 0) ringEl.style.stroke = 'var(--emerald)';
      else if (pct >= 50)            ringEl.style.stroke = 'var(--primary)';
      else                            ringEl.style.stroke = 'var(--amber)';
    }
  }
  ...
  taskAddForm && taskAddForm.addEventListener('submit', e => {
    e.preventDefault();
    ...
    // Switch to a filter where the new task is visible
    if (taskFilter === 'completed') {
      taskFilter = 'all';
      document.querySelectorAll('.task-filter-tab').forEach(t => t.classList.remove('active'));
      document.querySelector('.task-filter-tab[data-filter="all"]')?.classList.add('active');
    }

    showToast('✅ Task added', 1800);
  });

  /* ── List interactions: toggle complete / delete (delegated) ─ */
  const taskListEl = document.getElementById('taskList');

  taskListEl && taskListEl.addEventListener('click', e => {
    const card = e.target.closest('.task-card');
    if (!card) return;
    const id = card.dataset.id;

    /* Toggle complete */
    if (e.target.closest('.task-card__checkbox')) {
      ...
      if (task.completed) {
        ...
        setTimeout(() => renderTaskList(), 320);
        /* Achievements: track lifetime completed tasks */
        if (typeof registerTaskCompleted === 'function') registerTaskCompleted();
      } else {
        renderTaskList();
      }
      return;
  ...
  document.getElementById('clearCompletedBtn')?.addEventListener('click', () => {
    ...
    saveTasks(tasks);
    renderTaskList();
    showToast(`🧹 Cleared ${completedCount} completed task${completedCount > 1 ? 's' : ''}`, 2000);
  });
  ...
  const ACH_STORAGE_KEY      = 'studentHub_achievements_v1';
  const ACH_TASKS_TOTAL_KEY  = 'studentHub_tasksCompletedTotal_v1';

  /* ── Live source values (mirrors current dashboard stats) ── */
  const ACH_SOURCE = {
    attendanceStreakDays:   7,    // from Attendance module "Current Streak"
    bestProjectProgress:    91,   // highest project progress %
    productivityScore:      87,   // from KPI "Productivity Score"
  };

  /* ── Achievement definitions ───────────────────────────────
     target: value needed to unlock
     getValue: returns current progress value
     formatProgress: returns the progress label text
  ──────────────────────────────────────────────────────────── */
  const achievementDefs = [
    {
      id: 'streak',
      target: 7,
      getValue: () => ACH_SOURCE.attendanceStreakDays,
      formatProgress: (val, target, unlocked) => `${Math.min(val, target)} / ${target} days`,
    },
    {
      id: 'project',
      target: 100,
      getValue: () => ACH_SOURCE.bestProjectProgress,
      formatProgress: (val, target, unlocked) => unlocked ? `${target}% completed` : `${val}% best progress`,
    },
    {
      id: 'productivity',
      target: 85,
      getValue: () => ACH_SOURCE.productivityScore,
      formatProgress: (val, target, unlocked) => `${Math.min(val, 99)} / ${target}+`,
    },
    {
      id: 'tasks10',
      target: 10,
      getValue: () => getTasksCompletedTotal(),
      formatProgress: (val, target, unlocked) => `${Math.min(val, target)} / ${target} tasks`,
    },
  ];

  /* ── Lifetime tasks-completed counter ─────────────────────── */
  function getTasksCompletedTotal() {
    try {
      const raw = localStorage.getItem(ACH_TASKS_TOTAL_KEY);
      return raw === null ? 0 : parseInt(raw, 10) || 0;
    } catch (e) { return 0; }
  }
  function setTasksCompletedTotal(val) {
    try { localStorage.setItem(ACH_TASKS_TOTAL_KEY, String(val)); } catch (e) {}
  }

  /* Called from the task-management checkbox handler whenever a task
     transitions to "completed" */
  function registerTaskCompleted() {
    const total = getTasksCompletedTotal() + 1;
    setTasksCompletedTotal(total);
  ...
  function saveAchState(state) {
    try { localStorage.setItem(ACH_STORAGE_KEY, JSON.stringify(state)); } catch (e) {}
  }

  let achState = loadAchState();

  /* ── Core render / unlock-check ───────────────────────────── */
  function checkAndRenderAchievements(animateNew = true) {
    ...
    achievementDefs.forEach(def => {
      const card = grid.querySelector(`.ach-card[data-id="${def.id}"]`);
      if (!card) return;

      const value         = def.getValue();
      const wasUnlocked   = !!(achState[def.id] && achState[def.id].unlocked);
      const isUnlocked    = value >= def.target;

      if (isUnlocked) unlockedCount++;

      // Persist newly-unlocked state
      if (isUnlocked && !wasUnlocked) {
        achState[def.id] = { unlocked: true, unlockedAt: Date.now() };
        newlyUnlocked.push(def.id);
      } else if (!achState[def.id]) {
        achState[def.id] = { unlocked: isUnlocked, unlockedAt: isUnlocked ? Date.now() : null };
      }
      ...
      const progEl    = card.querySelector('[data-progress-text]');
      ...
      if (progEl) progEl.textContent = def.formatProgress(value, def.target, isUnlocked);
    });

    saveAchState(achState);

    /* Update summary ring + count */
    const countEl = document.getElementById('achUnlockedCount');
    const ringEl  = document.getElementById('achRingFill');
    if (countEl) animateAttnCounter ? animateAttnCounter(countEl, unlockedCount) : (countEl.textContent = unlockedCount);
    if (ringEl) {
      const CIRC = 113.1; // 2π × 18
      const pct  = unlockedCount / achievementDefs.length;
      setTimeout(() => { ringEl.style.strokeDashoffset = CIRC - pct * CIRC; }, 150);
      ringEl.style.stroke = unlockedCount === achievementDefs.length ? 'var(--emerald)' : 'var(--amber)';
    }

    /* Trigger unlock animation + toast for newly unlocked badges */
    if (animateNew && newlyUnlocked.length > 0) {
      newlyUnlocked.forEach((id, i) => {
        const card = grid.querySelector(`.ach-card[data-id="${id}"]`);
        const def  = achievementDefs.find(d => d.id === id);
        if (!card) return;

        setTimeout(() => {
          // Add "New" ribbon if not present
          if (!card.querySelector('.ach-card__new-tag')) {
            const tag = document.createElement('span');
            tag.className = 'ach-card__new-tag';
            tag.textContent = 'New!';
            card.appendChild(tag);
          }
          card.classList.add('just-unlocked');
          card.addEventListener('animationend', function handler(ev) {
            if (ev.animationName === 'achCardPulse') {
              card.classList.remove('just-unlocked');
              card.removeEventListener('animationend', handler);
            }
          });

          const title = card.querySelector('.ach-card__title')?.textContent || 'Achievement';
          const badgeEmoji = card.querySelector('.ach-card__badge')?.textContent || '🏆';
          showToast(`${badgeEmoji} Achievement unlocked: ${title}!`, 3200);
        }, i * 350);
      });
    }
  }

  /* ── Init achievements (lazy via IntersectionObserver) ────── */
  ...
  function initAchievements() {
    if (achInited) return;
    achInited = true;
    // On first render, don't fire "new" animations for already-stored unlocks
    checkAndRenderAchievements(false);
  }

  if (achSection && 'IntersectionObserver' in window) {
    const obs = new IntersectionObserver(entries => {
      entries.forEach(entry => { if (entry.isIntersecting) { initAchievements(); obs.unobserve(entry.target); } });
    }, { threshold: 0.1 });
    obs.observe(achSection);
  } else {
    setTimeout(initAchievements, 300);
  }
